...
4. **The inventory's `## Facts`** — what the
   ...
   `- Planned: <what> (<operator>, <date>)`, with
   `<operator>` as `rules/ssh-user.md` → Operator
   gives it, removed once it is done or the user
   drops it. Once written, the section leaves the
   inventory, which is then empty when no lead is
   left.
