...
- SSH CA signing keys, wherever they turn up
  (`rules/ssh-ca.md`). The certificates they sign,
  `*-cert.pub`, are public: `ssh-keygen -L` on them
  is fine
...
- Container secrets (`/run/secrets/*`) and
  container environment variables, which a bare
  `docker inspect` prints (`rules/containers.md`)
