...
- pfSense keeps the last 30 configs in `/cf/conf/backup/`.
  `rules/backups.md` still applies, `config.xml` included. If `/var`
  is a RAM disk (see Logs), keep the copies in
  `/root/hostwarden-backups/` instead.
...
- `/var` may be a RAM disk: logs from before an unclean restart may
  be gone. Not a finding on its own
