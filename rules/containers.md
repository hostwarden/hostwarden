...
- The `ls` loop also finds the accounts whose containers have all
  stopped. It reads every home in `passwd`, whatever the account's
  UID or shell: a service account below 1000 can own a store, a
  `nologin` account still owns one where a unit starts its
  containers, and a home outside `/home` is as good a place for it.
