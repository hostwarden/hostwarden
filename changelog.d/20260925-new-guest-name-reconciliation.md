...
- **A new guest could end up permanently registered under its
  ...
  first connection created carried that address, its `memory.md`
  title line included, and the host's `guests.md` linked to it that
  way for good. `hostwarden-new-guest` now reconciles the memory
  directory, its `memory.md` title line, `memory/ssh_hosts`, and
