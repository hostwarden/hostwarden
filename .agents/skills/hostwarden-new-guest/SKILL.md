...
5. **Verify the baseline** on the guest as `hostwarden-baseline`
   ...
   Where a host CA of the user's covers the guest, its host public
   keys go to the user to sign; where it trusts a user CA with a
   revocation list, the user hears that the guest must now be
   among the hosts their revocations reach (`rules/ssh-ca.md` →
