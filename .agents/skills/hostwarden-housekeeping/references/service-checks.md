...
host, at an address from `bind_hosts` and on `port`, or for a
container at the host address and port `docker ps` shows its
port 53 published on — `127.0.0.1` wherever the address is
`0.0.0.0`; use `nslookup` where `dig` is missing:
