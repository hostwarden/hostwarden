...
3. **Probe each host.** Hosts that time out or refuse the
   ...
   **In Claude Code, give each host its own subagent.**
   ...
   - **that its row comes back keyed** — every row key
     `references/probes.md` lists for those categories,
     spelled as there. The agent reads the keys from that
     file, since this session does not load it; agents told
     only "return the structured row" return four different
     shapes, and step 4 cannot build a column out of that.
     `unknown(needs-root)` is a value; a missing key is not;
