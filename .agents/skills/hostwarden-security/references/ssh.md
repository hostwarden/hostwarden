...
its `done`, where `OUT` holds each daemon's values, with the
privilege prefix at the top of that batch; and its
`cert-authority` grep in the call of its own that file gives. On
macOS only with Remote Login on. The severities are the ones
listed there. Report host and user
...
- It does not trust that user CA, or has no revocation list where
  ...
  for the user to add.
- No host certificate from that host CA → **INFO**, with its host
  public keys and names for the user to sign.
