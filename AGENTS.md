...
- **Absolute taboos (never run without explicit user request):** any
  ...
  re-quote a command to evade the guard. Legitimate exceptions (e.g.
  OS replacement) require the operator to set
  `HOSTWARDEN_GUARD_DISABLE=1` before launching the session.
